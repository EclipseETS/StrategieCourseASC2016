function [rad sunrise sunset] = solarradiationInstant(dem,lat,cs,r, currentDate)
...
%% Modified by Julien Longchamp, Éclipse 9, July 2016
%  Modification allows the function to compute the solar radiance for a
%  particular moment.
...
year = datevec(currentDate);
January1st = datenum(year(1),1,1,0,0,0); % DateNumber = datenum(Y,M,D,H,MN,S) % First of january of the current year
d = floor(currentDate - January1st); % Day of the year (fraction of 365)
dr= 0.0174532925;   % degree to radians conversion factor
hour = mod(currentDate, 1)*24; % Current hour as a fraction of 24h
...
% for d = 1:tau_a; 
    ...
    sunrise = 12*(1-mean(hsr(:))/pi); % Sunrise (fraction of 24h)
    sunset = 12*(1+mean(hsr(:))/pi); % Sunset (fraction of 24h)
%%  daily loop
    I=0;
%     for t=1:n:It % loop over sunshine hours
        ...
        % hourangle of sun hs  
%         hs=hsr-(pi*t/It);               % hs(t)
        hs=hsr-(pi*(hour-sunrise)/(sunset-sunrise));               % hs(t)
