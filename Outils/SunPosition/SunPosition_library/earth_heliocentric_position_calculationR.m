function earth_heliocentric_position = earth_heliocentric_position_calculationR(julian)
...
ntimes=length(JME);
earth_heliocentric_position.longitude=zeros(1,ntimes);
for ntime=1:ntimes
    % Compute the Earth Heliochentric longitude from the tabulated values.
    L0 = sum(A0.* cos(B0 + (C0 * JME(ntime))));
    L1 = sum(A1 .* cos(B1 + (C1 * JME(ntime))));
    L2 = sum(A2 .* cos(B2 + (C2 * JME(ntime))));
    L3 = sum(A3 .* cos(B3 + (C3 * JME(ntime))));
    L4 = sum(A4 .* cos(B4 + (C4 * JME(ntime))));
    L5 = A5 .* cos(B5 + (C5 * JME(ntime)));
    
    
    earth_heliocentric_position.longitude(ntime) = (L0 + (L1 * JME(ntime)) +...
        (L2 * JME(ntime)^2) + (L3 * JME(ntime)^3) +...
        (L4 * JME(ntime)^4) + (L5 * JME(ntime)^5)) / 1e8;
end
...
earth_heliocentric_position.latitude=zeros(1,ntimes);
for ntime=1:ntimes
    L0 = sum(A0 .* cos(B0 + (C0 * JME(ntime))));
    L1 = sum(A1 .* cos(B1 + (C1 * JME(ntime))));
    
    earth_heliocentric_position.latitude(ntime) = (L0 + (L1 * JME(ntime))) / 1e8;
end
...
earth_heliocentric_position.radius=zeros(1,ntimes);
for ntime=1:ntimes
    ...
    L2 = sum(A2 .* cos(B2 + (C2 * JME(ntime))));
    L3 = sum(A3 .* cos(B3 + (C3 * JME(ntime))));
    L4 = A4 .* cos(B4 + (C4 * JME(ntime)));
    
    % Units are in AU
    earth_heliocentric_position.radius(ntime) = (L0 + (L1 * JME(ntime))+...
        (L2 * JME(ntime)^2) + (L3 * JME(ntime)^3) +...
        (L4 * JME(ntime)^4)) / 1e8;
end