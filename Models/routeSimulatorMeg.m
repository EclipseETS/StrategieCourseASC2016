...
%     force_drag_roues(k) = Eclipse.nb_roue * 0.5 * constantes.mv_air * profil_vitesse(k-1).^2 * ((Eclipse.largeur_pneu*Eclipse.hauteur_roue^3)/(2 * Eclipse.rayon_roue^2));     % ****** À VÉRIFIER **********
    force_friction(k) = Eclipse.frottement; % ****** À VÉRIFIER **********       **********       **********        **********     **********       % ****** À VÉRIFIER **********
    ...
    if ((profil_vitesse(k-1) < newParcours.SpeedLimit(k)) || (profil_vitesse(k-1) < contraintes.vitesse_moy))    % Si la vitesse actuelle est inférieure à la vitesse de croisière et à la vitesse limite alors accélération
        ...
        force_traction_cible = force_opposition_tot(k) + (Eclipse.masse_totale * profil_accel(k)); % Force de traction nécessaire pour obtenir l'accélération désirée.
...
%         disp('Alerte : freinage')
    ...
    profil_force_traction(k) = min(force_traction_cible, Eclipse.couple_max*Eclipse.nb_moteur/Eclipse.rayon_roue); % La force de traction est plafonnée selon le couple max des deux moteurs d'Éclipse 9
    profil_accel(k) = (profil_force_traction(k)-force_opposition_tot(k))/Eclipse.masse_totale; % Le profil d'accélération réel est recalculé
