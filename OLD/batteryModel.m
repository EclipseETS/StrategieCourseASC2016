...
%  décharges de la cellule.
%  
%  Il est aussi possible d'utiliser le modèle sous forme de fonction pour
%  connaître la tension aux bornes de la batterie pour un état de charge et
%  un courant de sortie donné.
%  
...
%  Dernière modification : 17-01-2017 (JL) Modification du chemin pour la sauvegarde du fichier .mat
%%
...
% Sauvegarde les coefficients des polynômes si aucun argument n'est spécifié
if nargin == 0
    decharge0C2 = p1;
    decharge1C = p2;
    decharge2C = p3;
...
figure, hold on, title('ÉCLIPSE IX : Battery pack discharge')
plot(1-(discharge_0p2C(:,1)/Ccell), nb_cell_serie*discharge_0p2C(:,2), '*r')
plot(1-(continous_capacity/Ccell),nb_cell_serie*continous_discharge_0p2C, 'r')
plot(1-(discharge_1C(:,1)/Ccell), nb_cell_serie*discharge_1C(:,2), 'sb')
plot(1-(continous_capacity/Ccell),nb_cell_serie*continous_discharge_1C, 'b')
plot(1-(discharge_2C(:,1)/Ccell), nb_cell_serie*discharge_2C(:,2), '+g')
plot(1-(continous_capacity/Ccell),nb_cell_serie*continous_discharge_2C, 'g')
xlabel('SoC (%)')
ylabel('Voltage (V)')
legend('7.37 A (0.2C)', '36.85 A (1C)', '73.70 A (2C)')

figure, hold on, title('ÉCLIPSE IX : Cell discharge')
plot(1-(discharge_0p2C(:,1)/Ccell), discharge_0p2C(:,2), '*r')
plot(1-(continous_capacity/Ccell),continous_discharge_0p2C, 'r')
plot(1-(discharge_1C(:,1)/Ccell), discharge_1C(:,2), 'sb')
plot(1-(continous_capacity/Ccell),continous_discharge_1C, 'b')
plot(1-(discharge_2C(:,1)/Ccell), discharge_2C(:,2), '+g')
plot(1-(continous_capacity/Ccell),continous_discharge_2C, 'g')
xlabel('SoC (%)')
ylabel('Voltage (V)')
legend('7.37 A (0.2C)', '36.85 A (1C)', '73.70 A (2C)')
end
