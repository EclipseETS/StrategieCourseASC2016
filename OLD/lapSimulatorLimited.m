function lapLog = lapSimulator(parcours, etat_course, cellModel, strategy, eclipse9, constantes, reglement, meteo)
...
%    * eclipse9 -> Paramètres de la voiture solaire
...
%     
%     if (profil_vitesse(k-1) > parcours.speedLimit(k)/3.6)
%         disp('Speeding!')
%     end
    
    if (profil_vitesse(k-1) < strategy.vitesse_moy && profil_vitesse(k-1) < parcours.speedLimit(k)/3.6)     % Si la vitesse actuelle est inférieure à la vitesse de croisière alors accélération et à la limite de vitesse
        if (profil_vitesse(k-1) < strategy.vitesse_min)   % Si la vitesse actuelle est plus basse que la vitesse de croisière minimale (Typiquement au départ)
            profil_accel(k) = strategy.accel_max;   % Accélération forte (Départ)
        else
            profil_accel(k) = strategy.accel_nom;   % Accélération légère (En route)
        end        
    ...
    elseif (profil_vitesse(k-1) > strategy.vitesse_max)                                      % TODO : Ajouter une condition pour effectuer un freinage si la vitesse devient trop élevée (ie. descente de pente)
       profil_accel(k) = strategy.decel_nom;
       force_traction_cible = force_opposition_tot(k) + (eclipse9.masse_totale * profil_accel(k)); % Force de traction nécessaire pour obtenir l'accélération désirée.
