...
for k = 1:numel(list)
    ...
    data.DRIVE_BUS_VOLTAGE_ID = raw_data(:,1);
    data.DRIVE_BUS_CURRENT_ID = raw_data(:,2);
    data.DRIVE_MOTOR_VELOCITY_ID = raw_data(:,3);
    data.DRIVE_VEHICLE_VELOCITY_ID = raw_data(:,4);
    data.DRIVE_ODOMETER_ID = raw_data(:,5);
    data.DRIVE_DCBUS_AMPHOUR_ID = raw_data(:,6);
    data.BMS_PACK_CURRENT_ID = raw_data(:,7);
    data.BMS_PACK_VOLTAGE_ID = raw_data(:,8);
    data.BMS_AVERAGE_CELL_VOLTAGE_ID = raw_data(:,9);
    data.BMS_HIGHEST_TEMP_ID = raw_data(:,10);
    data.BMS_LOWEST_TEMP_ID = raw_data(:,11);
    data.BMS_REMAINING_ENERGY_ID = raw_data(:,12);
    data.MUPPET_UIN_MPPT1_ID = raw_data(:,13);
    data.MUPPET_IIN_MPPT1_ID = raw_data(:,14);
    data.MUPPET_UIN_MPPT2_ID = raw_data(:,15);
    data.MUPPET_IIN_MPPT2_ID = raw_data(:,16);
    data.MUPPET_UIN_MPPT3_ID = raw_data(:,17);
    data.MUPPET_IIN_MPPT3_ID = raw_data(:,18);
